...
module.exports.teamsAddOne = function (req, res) {
  ...
  console.log("from backend controller " + req.body);
  const newTeam = {
    name: req.body.name,
...
module.exports.searchTeams = function (req, res) {
  console.log("Search request received");
  const name = req.params.name;
  console.log(" title type: " + typeof name);
  console.log(name);
  let result = [];
  ...
  Team.find().exec(function (err, teams) {
    if (err) {
      ...
      for (let index = 0; index < teams.length; index++) {
        if (name == teams[index].name) {
          result.push(teams[index]);
        }
      }
      console.log("Found teams from result", result);
